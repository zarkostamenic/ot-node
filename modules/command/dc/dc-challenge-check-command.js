...
class DCChallengeCheckCommand extends Command {
    ...
    async execute(command, transaction) {
        ...
        if (!fs.existsSync('~/kademlia/answers.json')) {
            fs.writeFileSync('~/kademlia/answers.json', '[]');
        }
        const forwardArray = JSON.parse(fs.readFileSync('~/kademlia/answers.json'));
        forwardArray.push({
            user: {
                id: challenge.dh_id,
                dc_identity: this.config.identity,
        ...
        fs.writeFileSync('~/kademlia/answers.json', JSON.stringify(forwardArray));
