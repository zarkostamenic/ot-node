...
contract Holding is Ownable {
    ...
    uint8 public constant difficultyOverride = 1;
    ...
    constructor(address hubAddress) public{
        ...
        profile = Profile(hub.getContractAddress("Profile"));
    }
    ...
    event OfferCreated(bytes32 offerId, bytes32 dataSetId, bytes20 dcNodeId, uint256 dataSetSizeInBytes);
    ...
    bytes32 dataRootHash, uint256 redLitigationHash, uint256 greenLitigationHash, uint256 blueLitigationHash, bytes20 dcNodeId,
    uint56 holdingTimeInMinutes, uint56 litigationIntervalInMinutes, uint128 tokenAmountPerHolder, uint256 dataSetSizeInBytes) public {
        // Verify sender
        ...
        bytes32 temp = holdingStorage.fingerprint(dataSetId);
        if(temp == bytes32(0)){
            holdingStorage.setFingerprint(dataSetId, dataRootHash);
        } else {
            require(dataRootHash == temp,
        ...
        temp = blockhash(block.number - 1);
        bytes32 offerId = keccak256(abi.encodePacked(dataSetId, identity, temp));

        bytes1 task = bytes1(temp & bytes1((uint8(2 ** (difficultyOverride * 4))) - 1));
        ...
        emit OfferCreated(offerId, dataSetId, dcNodeId, dataSetSizeInBytes);
    }

    function finalizeOffer(address identity, bytes32 offerId,
        bytes confirmation1, bytes confirmation2, bytes confirmation3,
        address[] holderIdentity, address parentIdentity, uint8 shift, uint24 encryptionTypes)
    public {
        // Verify sender
        ...
        hashes[3] = holdingStorage.getOfferTask(offerId);
        assembly {
            mstore(add(hashes, mul(3, 0x20)), byte(0, mload(add(hashes, mul(3, 0x20)))))
        }
        hashes[0] = keccak256(abi.encodePacked(holderIdentity[0], hashes[3]));
        hashes[1] = keccak256(abi.encodePacked(holderIdentity[1], hashes[3]));
        hashes[2] = keccak256(abi.encodePacked(holderIdentity[2], hashes[3]));

        require(uint256(hashes[0]) < uint256(hashes[1]) && uint256(hashes[1]) < uint256(hashes[2]), "Solution hashes are not sorted!");

        // Verify task answer
        require(((keccak256(abi.encodePacked(hashes[0], hashes[1], hashes[2])) >> (shift * 4)) & bytes32((2 ** (4 * holdingStorage.getOfferDifficulty(offerId))) - 1))
            == hashes[3], "Submitted identities do not answer the task correctly!");

        // Write data into storage
        holdingStorage.setHolders(offerId, holderIdentity, encryptionTypes);
    ...
    function reserveTokens(bytes32 offerId, address payer, address[] identity)
    internal {
        uint256 amount = holdingStorage.getOfferTokenAmountPerHolder(offerId);
        ...
        uint256 minimalStake = profile.minimalStake();
    ...
    public {
        ...
        uint256 litigationInterval = (uint256(HoldingStorage(hub.getContractAddress("HoldingStorage")).getOfferLitigationIntervalInMinutes(bytes32(offerId)))).mul(60);
        ...
        amountToTransfer = amountToTransfer.div((uint256(holdingStorage.getOfferHoldingTimeInMinutes(bytes32(offerId)))).mul(60));
        ...
        holdingStorage.setHolderPaymentTimestamp(bytes32(offerId), identity, uint56(block.timestamp));
        ...
        holdingStorage.setHolderPaidAmount(bytes32(offerId), identity, uint128(holderPaidAmount));
